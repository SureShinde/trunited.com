...
RewardPointsSlider.prototype = {
    ...
    changePoint: function (points) {
        var truWallet = document.getElementById('truWallet_in_cart');
        if(typeof(truWallet) != 'undefined' && truWallet != null){
            var truWallet_in_cart = $('truWallet_in_cart').value;
            if(truWallet_in_cart == 1){
                $j = jQuery.noConflict();
                if(points > 0){
                ...
                $('reward_sales_point').value = 0;
                this.changePointCallback(0);
                return false;
            }
        }
