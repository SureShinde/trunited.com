function createForm(a, b) {
    var c = a.find(".get-started-btn")[0] || {}, d = (a.find(".close_btn")[0] || {},
    a.find("form")[0] || {}), e = $(d).find('input[name="vpath_newsletter_cb"]')[0] || {}, f = $(d).find('input[name="vpath_terms_cb"]')[0] || {}, g = $(d).find('input[type="email"]')[0] || {}, h = $(d).find('input[type="password"]')[0] || {}, i = a.find(".sou_loader")[0] || {}, j = function(a) {
        $(a).addClass("value-error"), b.value_error_on && b.value_error_on(a);
    }, k = function(a) {
        $(a).removeClass("value-error"), b.value_error_off && b.value_error_off(a);
    }, l = function(a) {
        for (var b in a) b = a[b], j(b);
    }, m = function(a, b) {
        var c = $(d).find("[name=" + a + "]")[0];
        if (void 0 !== c) {
            var e = $(c).data("errorplaceholder");
            if (void 0 !== e) {
                var f = $("#" + e);
                void 0 !== f && $(f).html(b[0]);
            }
        ...
        a.find("button, input, .form_row").prop("disabled", b).toggleClass("disabled", b);
    }, o = function() {
        a.find(".value-error").removeClass("value-error"), a.find(".error-placeholder").empty();
    }, p = function(a) {
        void 0 !== i && $(i).toggleClass("invisible", !a);
    }, q = function() {
        var a = [];
        return $(d).find("input").each(function() {
            void 0 !== $(this).data("required") && "checkbox" === $(this).attr("type") && ($(this).is(":checked") || ($(this).off("change").one("change", function() {
                k($(this));
            }.bind(this)), a.push(this)));
        }), a;
    }, r = function() {
        o(), n(!0);
        var a = window.$videopath.forms.getValues(d);
        a.newsletter = e.checked, a.terms = f.checked, a.username = a.email, delete a.vpath_terms_cb,
            delete a.vpath_newsletter_cb;
        var b = q();
        b.length > 0 ? (l(b), n(!1)) : (a.email = $(g).val(), a.username = a.email, a.password = $(h).val(),
            a.campaign = window.getValidCampaignParams(), a.referrer = window.getReferrerFromStorage(),
            p(!0), $videopath.api.auth.signup(window.signup_url, a, s, t, function() {
            k(f);
        }));
    }, s = function(a) {
        o(), n(!1), b.on_success && b.on_success(a);
    }, t = function(a) {
        p(!1);
        try {
            var b = JSON.parse(a.responseText);
            for (var c in b) m(c, b[c]);
            b.username && (b.email = b.username);
        } catch (a) {}
        $videopath.analytics.trackPage("web/signup_fail"), n(!1);
    };
    $(c).on("click", function() {
        r();
    });
...
function getURLParams() {
    try {
        ...
        return a ? JSON.parse('{"' + a.replace(/&/g, '","').replace(/=/g, '":"') + '"}', function(a, b) {
...
function getCampaignParamsFromURL() {
    var a, b, c = getURLParams(), d = {};
    for (a in utm_vars) b = utm_vars[a], c[a] && (d[b] = c[a]);
    for (a in vtm_vars) b = vtm_vars[a], c[a] && (d[b] = c[a]);
    return d;
}
...
function setCampaignParamsToStorage(a) {
    try {
        a.name && localStorage.setItem(utmStorageKey, JSON.stringify(a));
    } catch (a) {}
}

function saveCampaignParamsToStorageFromURL() {
    var a = getCampaignParamsFromURL();
    setCampaignParamsToStorage(a), console.log(a);
}

function getValidCampaignParams() {
    var a = getCampaignParamsFromURL();
    if (a.name) return a;
    var b = getCampaignParamsFromStorage();
    return !!b.name && b;
}

function saveReferrerToStorage() {
    var a = document.referrer;
    try {
        a.indexOf("videopath.com") == -1 && localStorage.setItem(referralStorageKey, a);
    } catch (a) {}
}
...
!window.addEventListener && function(a, b, c, d, e, f, g) {
    a[d] = b[d] = c[d] = function(a, b) {
        ...
        for (var c, d = 0; c = g[d]; ++d) if (c[0] == this && c[1] == a && c[2] == b) return this.detachEvent("on" + a, g.splice(d, 1)[0][3]);
    }, a[f] = b[f] = c[f] = function(a) {
        return this.fireEvent("on" + a.type, a);
    };
}(Window.prototype, HTMLDocument.prototype, Element.prototype, "addEventListener", "removeEventListener", "dispatchEvent", []);

var videopath_embed = videopath_embed || function(a, b, c) {
        function d(a) {
            var b = {
                id: a.getAttribute("data-videopath-id") || a.getAttribute("videopath-id"),
                retain_aspect: a.getAttribute("data-videopath-retain-aspect") || a.getAttribute("videopath-retain-aspect")
            };
            e(b);
        }
        function e(b) {
            a.videopath_lightbox.showVideo(b);
        }
        function f() {
            if (!g) {
                g = !0;
                for (var a = b.querySelectorAll(".videopath"), c = function(a) {
                    a.addEventListener("click", function() {
                        d(a);
                    }, !1);
                }, e = 0; e < a.length; e++) c(a[e]);
            }
        }
        var g = !1;
        return b.addEventListener("DOMContentLoaded", f), b.onreadystatechange = function() {
            "complete" == b.readyState && f();
        }, {
            showVideo: e,
            initialized: !0
        };
    }(window, document), videopath = {}, $videopath = $(videopath), $window = $(window);

!function() {
    var a = function() {
        var a = {};
        a.height = window.innerHeight || $window.height(), a.width = window.innerWidth || $window.width(),
            $videopath.winsize = a, $videopath.browsers.ios && window.top !== window ? $(".section-banner.banner-full-height").css("min-height", "600px") : $(".section-banner.banner-full-height").css("min-height", a.height + "px"),
            a.height > a.width ? ($("body").addClass("portrait"), $("body").removeClass("landscape")) : ($("body").addClass("landscape"),
                $("body").removeClass("portrait")), $videopath.trigger("resize", a);
    };
    $(document).ready(function() {
        $videopath.trigger("ready"), a(), $("#watch-video").addClass("vp_video_demo");
        var b = $("body");
        $window.scroll(function(a) {
            var c = $videopath.scrollTop = $window.scrollTop();
            b.toggleClass("scrolled_down", c > 20), $videopath.trigger("scroll");
        });
    }), $window.resize(a), $videopath.winsize = {
        width: $window.width(),
        height: $window.height()
    }, $videopath.scrollTop = $window.scrollTop();
}(), $videopath.constants = {
    SCREEN_XXS: 320,
    SCREEN_XS: 480,
    SCREEN_SM: 640,
    ...
    SCREEN_XXL: 1550,
    SCREEN_XXXL: 1900
}, $videopath.browsers = function() {
    var a = /gecko\/\d/i.test(navigator.userAgent), b = /MSIE \d/.test(navigator.userAgent), c = /Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(navigator.userAgent), d = b || c, e = d && (b ? document.documentMode || 6 : c[1]), f = /WebKit\//.test(navigator.userAgent), g = (f && /Qt\/\d+\.\d+/.test(navigator.userAgent),
        /Chrome\//.test(navigator.userAgent), /Opera\//.test(navigator.userAgent)), h = /Apple Computer/.test(navigator.vendor), i = (/Mac OS X 1\d\D([8-9]|\d\d)\D/.test(navigator.userAgent),
        /PhantomJS/.test(navigator.userAgent), /AppleWebKit/.test(navigator.userAgent) && /Mobile\/\w+/.test(navigator.userAgent)), j = i || /Android|webOS|BlackBerry|Opera Mini|Opera Mobi|IEMobile/i.test(navigator.userAgent), k = (i || /Mac/.test(navigator.platform),
        /win/i.test(navigator.platform), g && navigator.userAgent.match(/Version\/(\d*\.\d*)/));
    k && (k = Number(k[1])), k && k >= 15 && (g = !1, f = !0);
    return {
        gecko: a,
        ie: d,
        ie_version: e,
...
}(), $videopath.on("ready", function() {
    if (null != ga) {
        ga("require", "displayfeatures"), ga("require", "linker");
        var a = "";
        ga(function(b) {
            a = b.get("linkerParam");
        }), $videopath.analytics = function() {
            var a = function(a, b) {
                ga("merged.send", "pageview", a), ga("send", "pageview", a, {
                ...
                b || (b = $(document)), b.find("[data-conversion]").click(function(b) {
                    var c = $(b.currentTarget), d = c.data("conversion"), e = c.attr("href");
                    return a(d, e), !1;
                });
            }, c = "web/page" + window.location.pathname;
            return a(c), {
                trackPage: a,
                attach: b
            };
        }(), $videopath.analytics.attach();
    }
}), function() {
    var a = function() {
        var a = document.URL, b = a.indexOf("#"), c = b != -1 && a.substring(b + 1);
        if (c) {
            var d = $("[data-anchor='" + c + "']");
            d.length && (d = d.offset().top, $("html,body").scrollTop(d - 70));
        }
    };
...
}(), $videopath.api = {}, $videopath.api.auth = function() {
    var a = "https://api.videopath.com/v1/user/", b = "https://app.videopath.com/#token/", c = "?next=settings/plans/change/", d = "free", e = function(b, c, d, e, h) {
        window.optimizely.push([ "trackEvent", "submitInlineSignupForm" ]), $.ajax({
            type: "POST",
            url: a,
        ...
        window.optimizely.push([ "trackEvent", "signupSuccess" ]), $.when($.ajax({
            url: "https://analytics.twitter.com/i/adsct?txn_id=l62h4&p_id=Twitter&tw_sale_amount=0&tw_order_quantity=0"
        }), $.ajax({
            url: "https://t.co/i/adsct?txn_id=l62h4&p_id=Twitter&tw_sale_amount=0&tw_order_quantity=0"
        }), $.ajax({
            url: "https://www.facebook.com/tr?ev=6035483918688&amp;cd[value]=0.01&amp;cd[currency]=EUR&amp;noscript=1"
        })).always(function() {
            var f = $videopath.api.auth.context || {}, g = "";
            f.planid && f.planid != d && (g = c + f.planid), e.token_url = b + e.api_token_once + g,
                window.onMobile ? ($videopath.analytics.trackPage("web/signup_success"), a(e), window.alert("Thanks for signing up! The Videopath project builder can only be used on desktop browsers. You will receive an email shortly with a login link.")) : setTimeout(function() {
                    a(e), $videopath.analytics.trackPage("web/signup_success", e.token_url);
                }, 200);
        });
    }, g = function(a, b) {
        $videopath.analytics.trackPage("web/signup_fail"), a(b);
    };
    return {
        signup: e
    };
    ...
    clearErrors: function(a) {
        a = $(a), a.find(".error").removeClass("error"), a.find(".error_msg").empty();
    },
    setErrors: function(a, b) {
        if (this.clearErrors(a), b) {
            a = $(a);
            for (var c in b) {
                var d = b[c];
                Array.isArray(d) && (d = d[0]), a.find(":input[name=" + c + "]").addClass("error").parent().find(".error_msg").html(d);
            }
        }
    },
    getValues: function(a) {
        ...
        return a.find("input").each(function() {
            var a = $(this);
            c = a.attr("name"), b[c] = a.val();
        }), b;
    },
    clearForm: function(a) {
        a = $(a), a.find("input").each(function() {
            $(this).val("");
        });
    }
}, $videopath.helpers = {
    scaleIntoCenter: function(a, b, c) {
        var d = a.width(), e = a.height(), f = d / e, g = d, h = e;
        b > d && (g = b, h = b / f), c > e && (h = c, g = c * f), a.css({
            width: g + "px",
            height: h + "px",
            left: (b - g) / 2 + "px",
            top: (c - h) / 2 + "px"
        ...
        return this.each(function() {
            ...
            e ? i.find("*").addBack().each(function() {
                ...
                a(k).one(l, function b(e) {
                    ...
                    if (g++, d.apply(j.element, k), h.notifyWith(j.element, k), a(this).off(l, b), g == f) return c.call(i[0]),
                        h.resolveWith(i[0]), !1;
                }), k.src = j.src;
            });
        }), h.promise();
    ...
    if (!a.loaded) {
        ...
        b.async = !0, b.src = "//connect.facebook.net/en_US/fbds.js";
        var c = document.getElementsByTagName("script")[0];
        c.parentNode.insertBefore(b, c), a.loaded = !0;
    }
}(), window._fbq = window._fbq || [], window._fbq.push([ "track", "6022826707688", {
    value: "0.00",
    currency: "EUR"
} ]), $videopath.on("ready", function() {
    $(".block-more").hide(), $(".expand").click(function(a) {
        return $(".block-more").hide(), $(a.currentTarget).hide().next(".block-more").show(),
            !1;
    }), $(".collapse").click(function(a) {
        return $(".block-more").hide(), $(".expand").show(), !1;
    });
}), $videopath.on("ready", function() {}), $videopath.on("ready", function() {
    function a(a) {
        var b;
        return a && (b = a.split("."), b.pop(), b = b.join(".")), b;
    }
    var b, c, d, e, f, g = $(".block-demoscreen"), h = $(".home .banner-full-height"), i = h.find(".section-banner-video"), j = $(".block-testimonial .block-container:not(.block-count-1)");
    if ($(j).slick({
            dots: !0,
            arrows: !1,
            autoplay: !0,
            ...
            speed: 600,
            infinite: !0
        }), h.length && i.length && !$videopath.browsers.mobile && Modernizr.csstransforms) {
        var k = a(i.data("background-video-src"));
        if (k) {
            var l = '<video class="hidden" autoplay loop muted><source src="' + k + '.webm" type="video/webm"><source src="' + k + '.mp4" type="video/mp4"></video>', m = $(l).on("canplay", function(a) {
                m.removeClass("hidden");
                var b = !1, c = $('<button class="pause-button">').on("click", function() {
                    b = !b, c.toggleClass("paused", b), b ? m[0].pause() : m[0].play();
                }).appendTo(h);
            });
            i.replaceWith(m), h.find(".section-banner-image").addClass("hidden");
        }
    }
    var n = $(".product_arrow");
    ...
    var o, p, q, r = {
        ...
        SCREENS: 2,
        BOTTOM: 3
    }, s = 2560, t = $videopath.constants.SCREEN_MD, u = .29, v = 1400 / 980, w = {}, x = !1, y = !1;
    if (g.length) {
        b = g.find(".demoscreen-desktop-img-container"), c = g.find(".demoscreen-desktop-img"),
            d = g.find(".block"), f = $(".section-banner"), e = g.prevAll().add(f), p = b.length,
            b.each(function(a) {
                $(this).css("z-index", p - a);
            });
        var z = function() {
            c.each(function() {
                var a = $(this), b = a.data("img-url");
                a.css("background-image", "url(" + b + ")");
            });
        }, A = function(a) {
            var b = Math.max(Math.min(a, s), t), c = (a - t) / (s - t), d = (.5 - u) * c + u, e = (s - b) * d;
            return e;
        }, B = function(a) {
            var b = 0;
            w.top = e.map(function() {
                return $(this).outerHeight();
            }).get().reduce(function(a, b) {
                return a + b;
            }, 0), a.height < o && (b = (o - a.height) / 2, w.top += b), w.bottom = w.top + g.height() - o;
        }, C = function() {
            var a = $videopath.winsize;
            b.css({
                left: -A(a.width) + "px"
            }), x = !(a.width > t), Modernizr.csstransforms && (o = g.height() / p, B(a), x ? (b.css("top", ""),
                d.css("height", a.width * v + "px")) : (D(), d.css("height", ""))), x || y || (z(),
                y = !0);
        }, D = function() {
            if (!$videopath.browsers.mobile && !$videopath.browsers.safari && !x && Modernizr.csstransforms) {
                var a = $videopath.scrollTop, b = q;
                a < w.top ? q = r.TOP : a >= w.top && a < w.bottom ? (q = r.SCREENS, F(a - w.top)) : a >= w.bottom && (q = r.BOTTOM),
                b != q && E();
            }
        }, E = function() {
            q == r.TOP ? (g.removeClass("animating"), F(0), b.css("top", ""), G()) : q == r.SCREENS ? (g.addClass("animating"),
            $videopath.winsize.height < o && b.css("top", ($videopath.winsize.height - o) / 2 + "px")) : q == r.BOTTOM && (F(o * (b.length - 1)),
                g.removeClass("animating"), b.css("top", ""));
        }, F = function(a) {
            function d(b) {
                var d = Math.floor(a / o), e = o;
                b == d && (e = a - b * o), b > d && (e = 0), $(this).css({
        ...
        $videopath.on("resize", C), $videopath.on("scroll", D), C();
    }
}), $videopath.on("ready", function() {
    var a = window.location.pathname.substring(1);
    "" === a && (a = "welcome"), $(".menu_button").click(function() {
        $(".mobile_menu").toggleClass("showing");
    }), $(".mobile_menu a").click(function() {
        $(".mobile_menu").removeClass("showing");
    });
}), $videopath.on("ready", function() {
    $(".more-features-expander").on("change", function() {
        var a = $(this).attr("id"), b = $(this).parent().find("[for=" + a + "]"), c = this.checked, d = $(this).parent(), e = d.find(".option_item");
        e.each(function() {
            c === !0 ? $(this).hasClass("default") ? $(this).data("initial_visibility", !0) : $(this).addClass("default") : $(this).data("initial_visibility") || $(this).removeClass("default");
        }), c === !0 ? b.html("less features") : b.html("more features");
    });
}), $videopath.forms.signupForm = {
    create: createForm
}, $videopath.on("ready", function() {
    function a(a) {
        var b = function(b) {
            window.onMobile === !1 ? g(b.token_url) : a.remove();
        }, c = function(a) {
            "vpath_terms_cb" === $(a).attr("name") && ($(a).parent().parent().addClass("value-error shake"),
                setTimeout(function() {
                    $(a).parent().parent().removeClass("shake");
                }, 300));
        }, d = function(a) {
            "vpath_terms_cb" === $(a).attr("name") && $(a).parent().parent().removeClass("value-error");
        }, e = $(a.find("form")[0]), f = {
            on_success: b,
            value_error_on: c,
            value_error_off: d
        }, g = function(a) {
            e.addClass("hidden"), $(".suo_go_to_app_inline").removeClass("hidden"), $(".suo_go_to_app_inline .action a").attr("href", a);
        };
        $videopath.forms.signupForm.create(a, f);
    }
    $(".suo_content_inlined").each(function() {
        a($(this));
    });
}), $videopath.on("ready", function() {
    var a = function(a) {
        var b = function(b) {
            window.onMobile === !1 ? c(b.token_url) : a.remove();
        }, c = function(a) {
            $(".suo_signup_form").addClass("hidden"), $("#signup_overlay .suo_go_to_app .options a").attr("href", a),
                $(".suo_go_to_app").removeClass("hidden");
        }, d = function(a) {
            "vpath_terms_cb" === $(a).attr("name") && ($(a).parent().parent().addClass("shake value-error"),
                setTimeout(function() {
                    $(a).parent().parent().removeClass("shake");
                }, 300));
        }, e = function(a) {
    ...
    $(".suo_content_on_overlay").each(function() {
        a($(this));
    });
});

var overlay, contentArea, overlayTitle = {
    free: "Free Signup",
    "partners-pro-plus": "Pro Signup",
    "partners-starter": "Starter Signup"
}, overlaySubtitle = {
    free: "No credit card required",
    "pro-plus": "Payment will follow",
    starter: "Payment will follow",
    "partners-pro-plus": "Payment will follow",
    "partners-starter": "Payment will follow"
}, showPricingOverlay = function(a) {
    return a = a || {}, a.planid = a.planid || "free", $videopath.api.auth.context = a,
        "contact" == a.planid || "eur" == a.currency && "pro-plus" == a.planid ? void window.open("mailto:sales@videopath.com") : (overlay.find("h4").text(overlayTitle[a.planid]),
            overlay.find(".no-cc-notice").text(overlaySubtitle[a.planid]), window.optimizely.push([ "trackEvent", "showSignupOverlay" ]),
            $videopath.analytics.trackPage("web/planpick/popup"), overlay.removeClass("suo_removed"),
            void setTimeout(function() {
                overlay.removeClass("suo_hidden");
            }, 10));
}, hidePricingOverlay = function() {
    $videopath.analytics.trackPage("web/planpick/popdown"), overlay.addClass("suo_hidden"),
        setTimeout(function() {
            overlay.addClass("suo_removed");
        }, 300);
}, utm_vars = {
    utm_source: "source",
    utm_campaign: "name",
    ...
    vtm_medium: "medium",
    vtm_content: "content"
}, utmStorageKey = "vp_utm_params";

saveCampaignParamsToStorageFromURL();

var referralStorageKey = "vp_referrer";

saveReferrerToStorage(), $videopath.pricing_overlays = {
    setup: function() {
        overlay = $("#signup_overlay"), contentArea = $(".suo_content"), $(".signup.menu-item").click(function() {
            return $videopath.analytics.trackPage("web/navbar/signup"), showPricingOverlay(),
                !1;
        }), $(".suo_background, .close_btn").click(function() {
            return hidePricingOverlay(), !1;
        }), $(".login a").click(function() {
            return window.open("https://app.videopath.com"), !1;
        }), $(".section-banner-text .signup, .section-text .signup, button.signup, .block-pricing .block button").click(function(a) {
            var b = $(a.currentTarget);
            return showPricingOverlay(b.data()), !1;
        });
    }
}, $videopath.on("ready", function() {
    $videopath.pricing_overlays.setup();
}), $videopath.on("ready", function() {
    function a(a) {
        var b = "//player.videopath.com/" + a + "/";
        return screen.width < 600 || screen.height < 600 ? void (window.location.href = b) : void window.videopath_lightbox.showVideo({
            id: a,
            retain_aspect: 1.77,
            custom_url: b
        });
    }
    $(".vp_video_demo").click(function(b) {
        a("wmq2XEQj"), b.preventDefault(), window.optimizely.push([ "trackEvent", "showDemoVideo" ]);
    }), $(".vp_video").click(function(b) {
        a("wmq2XEQj"), b.preventDefault(), window.optimizely.push([ "trackEvent", "showDemoVideo" ]);
    });
});