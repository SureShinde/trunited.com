...
class Plumrocket_SocialLogin_Helper_Data extends Plumrocket_SocialLogin_Helper_Main
{
    ...
    /**
	 * All customer socials network
	 * @var Plumrocket_Pslogin_Model_Mysql4_Account_Collection
	 */
	protected $_accounts;
	...
	public function getPhotoPath($checkIsEnabled = true, $customerId = null, $type = null)
	{
		...
		if ($type !== null) {
			$path = Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . 'pslogin' . DS . 'photo' . DS . $type . DS . $customerId . '.' . Plumrocket_SocialLogin_Model_Account::PHOTO_FILE_EXT;
        	$pathUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'pslogin/photo/' . $type . DS . $customerId . '.' . Plumrocket_SocialLogin_Model_Account::PHOTO_FILE_EXT;
	...
	/**
	 * Retrieve customer networks
	 * @return Plumrocket_Pslogin_Model_Mysql4_Account_Collection
	 */
	...
	/**
	 * Retireve linking description
	 * @return string
	 */
	public function getLinkingDescription()
	{
		$process = Mage::helper('cms')->getBlockTemplateProcessor();
		return $process->filter(Mage::getStoreConfig('pslogin/link/description'));
	}
	 ...
	 * @return boolean
	 */
	public function isSocialLinkingEnabled()
	{
		return Mage::getStoreConfigFlag('pslogin/link/enable');
	}
}
