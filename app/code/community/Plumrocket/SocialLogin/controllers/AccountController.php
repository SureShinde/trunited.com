...
class Plumrocket_SocialLogin_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewAction()
    {
        ...
        if (!Mage::helper('pslogin')->isSocialLinkingEnabled()) {
    ...
    public function unlinkAction()
    {
        ...
        if ($this->getRequest()->getParam('id') && $customer && $customer->getId()) {
            $account = Mage::getModel('pslogin/account')->getCollection()
            ...
            if ($account->getId()) {
                try {
                    $account->delete();
                    Mage::getSingleton('customer/session')->addSuccess(Mage::helper('pslogin')->__('You was successfully unlinked'));
                } catch (Exception $e) {
                    Mage::getSingleton('customer/session')->addError($e->getMessage());
                }
            }
        } else {
            Mage::getSingleton('customer/session')->addError(Mage::helper('pslogin')->__('Missed required parameters'));
        }
    ...
    public function linkAction()
    {
        ...
        $redirectUrl = Mage::getUrl('pslogin/account/view');
        $this->getResponse()->setBody($this->_jsWrap('if (window.opener && window.opener.location &&  !window.opener.closed) { window.close(); window.opener.location.href = "'.$redirectUrl.'"; } else { window.location.href = "'.$redirectUrl.'"; }'));
    }
}