...
class CommerceExtensions_Orderimportexport_Model_Profile extends Mage_Dataflow_Model_Profile
{
...
//            $p['file']['path'] = 'var/export';
            ...
            $fileXml .= '    <var name="filename"><![CDATA[' . $p['file']['filename'] . ']]></var>' . $nl;
   			$fileXml .= '    <var name="link">'.Mage::getBaseUrl().'orderimportexport?download_file=' . $p['file']['filename'] . '</var>' . $nl;
...
//            $parseDataXml .= '</action>'.$nl.$nl;
            ...
            $parseDataXml = '<action type="commerceextensions_orderimportexport/convert_parser_exportorders" method="unparse">' . $nl;
            $parseDataXml .= '    <var name="store"><![CDATA[' . $this->getStoreId() . ']]></var>' . $nl;
            ...
            $filter_by_payment_method = isset($p['unparse']['filter_by_payment_method']) ? $p['unparse']['filter_by_payment_method'] : '';
            $parseDataXml .= '    <var name="filter_by_payment_method"><![CDATA[' . $filter_by_payment_method . ']]></var>' . $nl;
            ...
            $exportProductTaxPercent = isset($p['unparse']['export_product_tax_percent']) ? $p['unparse']['export_product_tax_percent'] : 'false';
            $parseDataXml .= '    <var name="export_product_tax_percent"><![CDATA[' . $exportProductTaxPercent . ']]></var>' . $nl;
			
           $exportOrderAddressAlways = isset($p['unparse']['export_order_address_always']) ? $p['unparse']['export_order_address_always'] : 'false';
           $parseDataXml .= '    <var name="export_order_address_always"><![CDATA[' . $exportOrderAddressAlways . ']]></var>' . $nl;
        ...
        if ($import) {
            ...
            $entityXml = '<action type="commerceextensions_orderimportexport/convert_adapter_exportorders" method="load">' . $nl;
            $entityXml .= '    <var name="store"><![CDATA[' . $this->getStoreId() . ']]></var>' . $nl;
        ...
        if ($import) {
            ...
            $useHistoricTax = isset($p['parse']['use_historic_tax']) ? $p['parse']['use_historic_tax'] : 'false';
            $parseFileXmlInter .= '    <var name="use_historic_tax"><![CDATA[' . $useHistoricTax . ']]></var>' . $nl;
            
            $skipProductLookup = isset($p['parse']['skip_product_lookup']) ? $p['parse']['skip_product_lookup'] : 'false';
            $parseFileXmlInter .= '    <var name="skip_product_lookup"><![CDATA[' . $skipProductLookup . ']]></var>' . $nl;
            
            if ($this->getDataTransfer()==='interactive') {
                $xml = $parseFileXmlInter;
                $xml .= '    <var name="adapter">commerceextensions_orderimportexport/convert_adapter_importorders</var>' . $nl;
