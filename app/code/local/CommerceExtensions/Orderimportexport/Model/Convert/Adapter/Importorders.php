...
//        }
...

	{
        ...
        $invoiceTypeId = Mage::getModel('eav/entity')->setType('invoice')->getTypeId();
		...
		//update increment ID for invoice
		/*
        if(isset($importData['store_id']) && isset($importData['invoice_id']) && $orderTypeId != "" && $importData['invoice_id'] !="") {
            $finalinvoiceID = $importData['invoice_id'] - 1;
            $write->query("UPDATE `".$prefix."eav_entity_store` SET increment_last_id = '".$finalinvoiceID."' WHERE store_id = '". $importData['store_id'] ."' AND entity_type_id = '". $invoiceTypeId ."'");
		...
		*/
        ...
        foreach ($products_ordered as $data) {
			...
			if ($this->getBatchParams('skip_product_lookup') == "true") {
				...
				$product = Mage::getModel('catalog/product')->loadByAttribute('sku', $keyforinsert);
			} else {
    ...
    public function processOrder(&$orderData, $importData, $write, $prefix, $products_ordered, $read)
    {
        ...
        try {
			...
			#$this->updateOrderCommentsHistory($importData, $prefix, $order1, $write);
...

    {
		...
		if ($this->getBatchParams('skip_product_lookup') == "true") {
			...
			$product2 = Mage::getModel('catalog/product')->loadByAttribute('sku', $key);
		} else {	
	...
	public function updateOrderCommentsHistory($importData, $prefix, $order1, $write)
    {
		if(isset($importData['additional_comments'])) {
			...
			foreach($parts as $individual_comment)
			{
				...
				$insertComment = "INSERT INTO `".$prefix."sales_flat_order_status_history` (parent_id, is_customer_notified, is_visible_on_front, comment, status, created_at) VALUES (\"".$order1->getId()."\",\"".$comment[0]."\",\"".$comment[1]."\",\"" . addslashes($comment[2])."\",\"".$comment[3]."\",\"".$comment[4]."\")";
				
				$write->query($insertComment);
				
	
				} catch (Exception $e) {
	
					echo "Order #" . $importData['order_id'] . " ERROR ADDING ORDER COMMENTS: " . $e->getMessage();
