    ...
    extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function unparse()
    {
		...
		*/
			...
			if($this->getVar('export_order_address') == "true") {
		   ...
		   $select_qry = $read->query("SELECT tax_percent FROM `".$prefix."sales_flat_order_item` WHERE order_id ='".$order->getId()."' LIMIT 1");
            ...
            $row['base_shipping_tax_refunded'] = $order->getData('base_shipping_tax_refunded');
						...
						foreach ($items as $itemId => $item)
						{
							 ...
							 */
							 ...
							 if ($item->getProductType() == "bundle") {
								 ...
								 #$productbundleItem = Mage::getResourceModel('catalog/product')->getProductsSku($order_prodID);
								 #$productbundleItemSku = $productbundleItem[0]['sku'];
						...
						$all_comments = "";
						$select_qry = "SELECT * FROM `".$prefix."sales_flat_order_status_history` WHERE parent_id = '".$order->getId()."'";

						$rows = $read->fetchAll($select_qry);
			
						foreach($rows as $newrowData)
						{ 
							$all_comments .= $newrowData['is_customer_notified'] . "^^" . $newrowData['is_visible_on_front']. "^^" . $newrowData['comment']. "^^" . $newrowData['status']. "^^" . $newrowData['created_at'] . "||";
						}
