...
class Indies_Recurringandrentalpayments_Model_Payment_Method_ProtxDirect extends Indies_Recurringandrentalpayments_Model_Payment_Method_Abstract
{
    ...
    {
        Mage::getModel('recurringandrentalpayments/protxDirect')
    ...
    public function processOrder(Mage_Sales_Model_Order $PrimaryOrder, Mage_Sales_Model_Order $Order = null)
    {
        ...
        $model = Mage::getModel('Recurringandrentalpayments/protxDirect')->load($this->getSubscription()->getId(), 'subscription_id');
        ...
        if (empty($ready)) {
            throw new Indies_Recurringandrentalpayments_Exception($this->__("Order cannot be completed. Unknown error"));
        }

        if ($ready['Status'] != 'OK') {
            throw new Indies_Recurringandrentalpayments_Exception($ready['Status'] . " - " . $ready['StatusDetail']);
        }
    }
