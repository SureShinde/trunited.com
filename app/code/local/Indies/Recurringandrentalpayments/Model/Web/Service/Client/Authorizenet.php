...
class Indies_Recurringandrentalpayments_Model_Web_Service_Client_Authorizenet extends Indies_Recurringandrentalpayments_Model_Web_Service_Client
{
    ...
    {
        if (Mage::getStoreConfig(Indies_Recurringandrentalpayments_Model_Payment_Method_Authorizenet::XML_PATH_AUTHORIZENET_SOAP_TEST)) {
    ...
    public function getApiLoginId()
    {
        ...
        return Mage::getStoreConfig(Indies_Recurringandrentalpayments_Model_Payment_Method_Authorizenet::XML_PATH_AUTHORIZENET_API_LOGIN_ID, $_storeId);
    }
    ...
    public function getTransactionKey()
    {
        ...
        if($this->getSubscription()->getStoreId())
            $_storeId = $this->getSubscription()->getStoreId();
        return Mage::getStoreConfig(Indies_Recurringandrentalpayments_Model_Payment_Method_Authorizenet::XML_PATH_AUTHORIZENET_TRANSACTION_KEY, $_storeId);
    }
    ...
    {
        return Mage::getStoreConfig(Indies_Recurringandrentalpayments_Model_Payment_Method_Authorizenet::XML_PATH_AUTHORIZENET_PAYMENT_ACTION);
    }
    ...
    public function createCIMAccount()
    {
        ...
        if ($result = $this->_runRequest('CreateCustomerProfile', array(self::ERR_CODE_DUPLICATE_CUSTOMER))) {
            ...
            if ($result instanceof Indies_Recurringandrentalpayments_Model_Web_Service_Client_Authorizenet_Error) {
                ...
                if ($result->getCode() == self::ERR_CODE_DUPLICATE_CUSTOMER) {
                    ...
                    if (preg_match("/\s+([0-9]+)\s+/", $result->getText(), $matches)) {
                        ...
                        throw Indies_Recurringandrentalpayments_Exception("Customer duplicate responded but can't find customer id in reply");
                    }
                }
            } else {
    ...
    protected function _addAdditionalCard(&$cimAccount)
    {
        ...
        $creditCardNumberShort = substr(Mage::getSingleton('customer/session')->getRecurringandrentalpaymentsCcNumber(), -4);
                               ...
                               'transaction' => array(
                                   'profileTransAuthOnly' => array(
                                       ...
                                       'order' => array(
                                           ...
                                           'description' => Mage::helper('recurringandrentalpayments')->__("Subscription #%s payment", $this->getSubscription()->getId())
                                       )
                                   )
                               )
                          ));
    ...
    public function createAuthCaptureTransaction()
    {
		...
		if (isset($getSession))
		{
			$grand_total = 	$this->getOrder()->getGrandTotal() - $getSession ;
		}
    ...
    protected function _runRequest($name, $notFatalErrors = null)
    {
        try {
		    ...
		    if ($result) {
				...
				$this->getSubscription()->setResponsemessage($result->$resultName->messages->MessagesTypeMessage->text);
                ...
                if (strtolower($result->$resultName->resultCode) == 'error') {
					...
					throw new Exception("Authorize.net[{$resultName}] responded error #{$result->$resultName->messages->MessagesTypeMessage->code} '{$result->$resultName->messages->MessagesTypeMessage->text}'");
                    return false;
                } else {
                    // All okay
					...
					$this->getSubscription()->setResponsemessage('Success');
                    return $result->$resultName;
                }
            } else {
				$this->getSubscription()->setResponsemessage('Authorize.net returned empty result ');
                throw new Exception("Authorize.net returned empty result");
            }
        } catch (Exception $e) {
            if ($e instanceof Indies_Recurringandrentalpayments_Exception) {
            } elseif ($e instanceof Exception) {
                throw new Mage_Checkout_Exception($e->getMessage());
            } else {
				$this->getSubscription()->setResponsemessage($e->getMessage());
                throw $e;
            }
        }
    }
     ...
     * This is not currently used because CIM is more useful for recurringandrentalpayments
     * @return Indies_Recurringandrentalpayments_Model_Web_Service_Client_Authorizenet
     */
    public function createSubscription()
    {
        ...
        if ($lastPaidDate = $this->getSubscription()->getLastPaidDate()) {
            ...
            foreach (Mage::getModel('recurringandrentalpayments/sequence')
                    ->getCollection()
                ...
                $start_date = new Zend_Date($Item->getDate(), Indies_Recurringandrentalpayments_Model_Subscription::DB_DATE_FORMAT);
            }
        }
        ...
        try {
            ...
            if ($result) {
                if (strtolower($result->ARBCreateSubscriptionResult->resultCode) == 'error') {
                    ...
                    throw new Exception("Authorize.net responded error #{$result->ARBCreateSubscriptionResult->messages->MessagesTypeMessage->code} #{$result->ARBCreateSubscriptionResult->messages->MessagesTypeMessage->text}");
                } else {
                ...
                throw new Indies_Recurringandrentalpayments_Exception("Authorize.net returned empty result");
            }
        } catch (Exception $e) {
            if ($e instanceof Indies_Recurringandrentalpayments_Exception) {
     ...
     * @return
     */
    protected function _getTotalOccurrences(Indies_Recurringandrentalpayments_Model_Subscription $Subscription)
    {
        if ($Subscription->isInfinite()) {
            return 9999;
        } else {
            // Calculate how much subscription events are generated
            return Mage::getModel('recurringandrentalpayments/sequence')->getCollection()->addSubscriptionFilter($Subscription)->count();
        }
    }

    /**
     * Converts recurringandrentalpayments period to ARB period array
     * @param Indies_Recurringandrentalpayments_Model_Period $Period
     * @return array
     */
    protected function _convertPeriod(Indies_Recurringandrentalpayments_Model_Period $Period)
    {
        $unitMultiplier = 1;
        $unit = $Period->getPeriodValue();
        switch ($Period->getPeriodType()) {
            case Indies_Recurringandrentalpayments_Model_Source_Periods::PERIOD_WEEKS:
                $unitMultiplier = 7;
                $unit = self::UNIT_DAYS;
                break;
            case  Indies_Recurringandrentalpayments_Model_Source_Periods::PERIOD_YEARS:
                $unitMultiplier = 12;
                $unit = self::UNIT_MONTHS;
                break;
            case  Indies_Recurringandrentalpayments_Model_Source_Periods::PERIOD_MONTHS:
                $unitMultiplier = 1;
                $unit = self::UNIT_MONTHS;
                break;
            case Indies_Recurringandrentalpayments_Model_Source_Periods::PERIOD_DAYS:
                $unitMultiplier = 1;
                $unit = self::UNIT_DAYS;
                break;
        }
    ...
    {
	    $cardNumber = Mage::getSingleton('customer/session')->getRecurringandrentalpaymentsCcNumber();
        $cid = Mage::getSingleton('customer/session')->getRecurringandrentalpaymentsCcCid();
        $an_payment = array(
            'creditCard' => array(
    ...
    {
        $obj = new Indies_Recurringandrentalpayments_Model_Web_Service_Client_Authorizenet_Error;
