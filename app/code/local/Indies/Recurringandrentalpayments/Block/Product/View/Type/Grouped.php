...
class Indies_Recurringandrentalpayments_Block_Product_View_Type_Grouped extends Mage_Catalog_Block_Product_View_Type_Grouped
{
...
/*    public function getAssociatedProducts()
    {
		...
		$plan = Mage::getModel('recurringandrentalpayments/plans')->load($plans_product->getPlanId(),'plan_id');
			...
			->addFieldToFilter('plan_id',
										Mage::getModel('recurringandrentalpayments/plans_product')
										->load($products[0]->getId(),'product_id'));
        ...
        {
            if ($this->_checkPeriods($mainProductPeriods, $product->getIndiesPeriod())) {
                $readyArray[] = $product;
            }
        }
        return $readyArray;
