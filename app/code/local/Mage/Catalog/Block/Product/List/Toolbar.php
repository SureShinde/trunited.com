...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
    ...
    public function setCollection($collection)
    {
        ...
        if ($this->getCurrentOrder())
        {
            ...
            if(isset($postData) && $postData!= '' && trim($postData) !='ALL')
            {
                ...
                if(isset($keywordData) && $keywordData != '')
                    $this->_collection->addAttributeToFilter(array(
                        array('attribute'=>'name', 'like'=>'%'.$keywordData.'%')
                    ));

            } else if(isset($keywordData) && $keywordData != '') {
                if($keywordData != '')
                    $this->_collection->addAttributeToFilter(array(
                        array('attribute'=>'name', 'like'=>'%'.$keywordData.'%')
                    ));
    ...
    public function getNewUrl($postData, $keywordData, $originUrl)
    {
        $new_url = $originUrl;
        if($postData != null)
            $new_url = $originUrl.'&alpha='.$postData;

        if($keywordData != null)
            $new_url = $originUrl.'&quick_keyword='.$keywordData;
