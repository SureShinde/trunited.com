...
//            zend_debug::dump($inv->debug());
            $invoice_qty += $inv->getTotalQty();
        }
        zend_debug::dump($invoice_qty);
        zend_debug::dump($total_qty_ordered);
        zend_debug::dump($total_qty_ordered == $invoice_qty);
        ...
        zend_debug::dump($shipment_qty);
        zend_debug::dump($total_qty_ordered);
        zend_debug::dump($total_qty_ordered == $shipment_qty);
...
//		$transactionSave->save();
	...
	public function insertMobileCodeAction() {
		...
		$installer->run("
			...
			DROP TABLE IF EXISTS {$setup->getTable('mobile_codes')};
			CREATE TABLE {$setup->getTable('mobile_codes')} (
...
INSERT INTO {$setup->getTable('mobile_codes')} (`id`, `iso`, `name`, `nicename`, `iso3`, `numcode`, `phonecode`) VALUES
