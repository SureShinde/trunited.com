        ...
        foreach ($rules as $rule) {
            ...
            if ($rule->validate($product)) {
                $points += $this->calcCatalogPoint(
                        ...
                        Mage::helper('rewardpoints/calculator')->getPrice($product, $product->getFinalPrice(), $earnPointByTax),
                        Mage::helper('rewardpoints/calculator')->getPrice($product, $product->getFinalPrice(), $earnPointByTax) - $product->getCost(),
//                                $product->getPrice(),
    ...
    public function getShoppingCartPoints($quote, $customerGroupId = null, $websiteId = null, $date = null) {
        ...
        foreach ($rules as $rule) {
            ...
            $pointsRule = $this->calcSalesPoints(
                $rule->getSimpleAction(),
                $rule->getSimpleAction() == 'by_percent' ? $percent_points : $rule->getPointsEarned(),
                $rule->getMoneyStep(),
                $rowTotal, $rule->getQtyStep(), $qtyTotal, $rule->getMaxPointsEarned()
            );
            $points += $pointsRule;
			if($points >= 1){
				$rule->setUsed(1);
