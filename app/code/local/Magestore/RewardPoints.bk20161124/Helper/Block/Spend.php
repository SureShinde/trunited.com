...
class Magestore_RewardPoints_Helper_Block_Spend extends Magestore_RewardPoints_Helper_Calculation_Abstract
{
    ...
    public function getRulesJson($rules = null) {
        ...
        foreach ($rules as $rule) {
            $ruleOptions = array();
            if ($this->getCustomerProductCredit() < $rule->getPointsSpended()) {
                $ruleOptions['optionType'] = 'needPoint';
                $ruleOptions['needPoint'] = $rule->getPointsSpended() - $this->getCustomerProductCredit();
            } else {
                $quote = $this->getQuote();
                $sliderOption = array();
...
//                $maxPoints = $this->getCustomerPoint();
                $maxPoints = $this->getCustomerProductCredit();
...
//                $maxPoints = $this->getCustomerPoint();
    ...
    public function getCustomerProductCredit()
    {
        return Mage::helper('rewardpoints/customer')->getProductCredit();
    }
    ...
    // {
        // if (!$this->hasCache('customer_point')) {
            // $points  = $this->getCustomerProductCredit();
            // $points -= $this->getCalculation()->getPointItemSpent();
            // $points -= $this->getCalculation()->getCheckedRulePoint();
            // if ($points < 0) {
                // $points = 0;
