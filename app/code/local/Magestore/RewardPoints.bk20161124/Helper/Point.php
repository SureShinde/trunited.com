...
class Magestore_RewardPoints_Helper_Point extends Mage_Core_Helper_Abstract
{
    ...
    const XML_PATH_PRODUCT_CREDIT_NAME      = 'rewardpoints/general/product_credit_name';
    const XML_PATH_PRODUCT_CREDIT_PLURAL    = 'rewardpoints/general/product_credit_names';
    ...
    public function getProductCreditName($store = null)
    {
        if ($pointName = trim(Mage::getStoreConfig(self::XML_PATH_PRODUCT_CREDIT_NAME, $store))) {
            return $pointName;
        }
        return $this->__('Product Credit');
    }
    ...
    public function getProductCreditPluralName($store = null)
    {
        if ($pluralName = trim(Mage::getStoreConfig(self::XML_PATH_PRODUCT_CREDIT_PLURAL, $store))) {
            return $pluralName;
        }
        return $this->__('Product Credits');
    }
    ...
    public function formatProductCredit($points, $store = null)
    {
        ...
        if (abs($points) <= 1) {
            return $points . ' ' . $this->getProductCreditName($store);
        }
        return $points . ' ' . $this->getProductCreditPluralName($store);
    }
