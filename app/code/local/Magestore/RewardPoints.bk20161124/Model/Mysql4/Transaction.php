...
class Magestore_RewardPoints_Model_Mysql4_Transaction extends Mage_Core_Model_Mysql4_Abstract {
    ...
    public function updatePointUsed($transaction) {
        $totalAmount = -$transaction->getProductCredit();
        if ($totalAmount <= 0) {
            return $this;
        }
        ...
        $selectSql = $read->select()->reset()
                ->from(array('t' => $this->getMainTable()), array('transaction_id', 'point_amount', 'point_used', 'product_credit'))
                ->where('customer_id = ?', $transaction->getCustomerId())
                ->where('product_credit > point_used')
        ...
        foreach ($trans as $tran) {
            $availableAmount = $tran['product_credit'] - $tran['point_used'];
        ...
        if (count($usedIds)) {
            $write->update($this->getMainTable(), array(
                'point_used' => new Zend_Db_Expr('product_credit')
                    ), array(
