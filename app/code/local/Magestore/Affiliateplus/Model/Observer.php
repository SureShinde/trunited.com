...
//            $affiliateplusCommissionItem = $commissionObj->getAffiliateplusCommissionItem();
            ...
            $defaultAmount = 0;
            $defCommission = 0;
            $rwEarningPoint = $commissionObj->getRwEarningPoint();
...
//            if ($commissionValue && $commissionObj->getDefaultCommission()) {
            if ($commissionObj->getDefaultCommission()) {
                if ($commissionType == 'percentage') {
                    if ($commissionValue > 100)
                        $commissionValue = 100;
                    if ($commissionValue < 0)
                ...
                foreach ($order->getAllItems() as $item) {
                    ...
                    if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                        ...
                        foreach ($item->getChildrenItems() as $child) {
                            ...
                            $defaultItemIds[] = $child->getProduct()->getId();
                            $defaultItemNames[] = $child->getName();
                            $rwEarningPoint += $child->getRewardpointsEarn();
                        }
                        // if ($childHasCommission) {
                        // $orderItemIds[] = $item->getProduct()->getId();
                        // $orderItemNames[] = $item->getName();
                        ...
                        $defCommission += $commissionObj->getCommission();
                        $defaultAmount += $item->getBasePrice();
                        $rwEarningPoint += $item->getRewardpointsEarn();
                    }
                }
            }
//            if (!$baseDiscount && !$commission)
//                return $this;
            if($rwEarningPoint > 0) {
                ...
                $transactionData = array(
                    ...
                    'default_amount' => $defaultAmount,
                    'type' => 3,
//                'rw_earning_point' => $rwEarningPoint
                );
                if ($account->getUsingCoupon()) {
                    $session = Mage::getSingleton('checkout/session');
                    $transactionData['coupon_code'] = $session->getData('affiliate_coupon_code');
...
//        }
    ...
    public function customerSaveAfter($observer) {
        ...
        //customize by vietbq check coupon code when register customer

        $affiliateId = $request->getPost('affiliate_id');
    ...
    //    customize by vietbq check coupon code when register customer
    public function AddFieldAffiliateCustomerRegister($obj){
        ...
        $html = $transport->getHtml ();
        $st_html = $block->getChildHtml ( 'customer_register_affiliate' );
...
//    end customize by vietbq check coupon code when register customer
	...
	public function customerRegisterSuccess($observer)
    {
        ...
        if($data['no_refers_me'] != null && $data['no_refers_me'] == 1){
            $customer->setData('no_refers_me',1);
        }
		$customer->setData('referred_affiliate_name',$data['affiliate_name'] != '' ? $data['affiliate_name'] : '');
		$customer->save();
    }
}
