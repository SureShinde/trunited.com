...
//                $account->setBalance($account->getBalance()+$additionalCommission)->save();
        ...
        try {
                ...
                if ($this->getStatus() != 1) {    // Changed By Adam to solve the problem
                    foreach ($order->getAllItems() as $item) {
                        ...
                        if($item->getRewardpointsEarn()){
                            $totalRewardPoint = $item->getRewardpointsEarn();
                            $transactionRewardPoint += $totalRewardPoint * ($item->getQtyInvoiced() - $item->getQtyRefunded()) / $item->getQtyOrdered();
                            zend_Debug::Dump($transactionRewardPoint);
                        }
            ...
            if ($commission || $transactionRewardPoint) {
                $status = 1;
                ...
                if ($transactionRewardPoint) {
                    $this->setRwEarningPoint($transactionRewardPoint);
                }
...
//    }
    ...
    public function reduce($creditmemo) {
        if ($this->getStatus() == 1) {
            ...
            $reduceTransactionRewardPoint = 0;
            foreach ($creditmemo->getAllItems() as $item) {
                ...
                if (!$item->getAffiliateplusCommissionFlag()) {
                    ...
                    if($orderItem->getRewardpointsEarn()){
                        $orderItemQty = $orderItem->getQtyOrdered();
                        $orderItemRewardPoint = (float) $orderItem->getRewardpointsEarn();
                        if ($orderItemRewardPoint && $orderItemQty) {
                            $reduceTransactionRewardPoint += $orderItemRewardPoint * $item->getQty() / $orderItemQty;
                        }
                    }
                }
            }
...
//            }
            ...
            try {
               ...
               if ($reduceTransactionCommission) {
                ...
                //substract number point in manage transaction
                if ($reduceTransactionRewardPoint) {
                    if ($this->getRwEarningPoint() <= $reduceTransactionRewardPoint && $order->getBaseSubtotal() == $order->getBaseSubtotalRefunded()) {
                        $this->setRwEarningPoint(0)
                            ->setStatus(3)
                        ;
                    } else {
                        $this->setRwEarningPoint($this->getRwEarningPoint() - $reduceTransactionRewardPoint);
                    }
                }
                $this->save();
