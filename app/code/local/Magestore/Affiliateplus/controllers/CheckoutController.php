...
class Magestore_Affiliateplus_CheckoutController extends Mage_Core_Controller_Front_Action
{
	...
	public function saveRefersAction()
	{
		$data = $this->getRequest()->getParams();
		Mage::getSingleton('core/session')->setWRMName($data['name']);	
		Mage::getSingleton('core/session')->setWRMId($data['id']);	
		Mage::getSingleton('core/session')->setWRMNo($data['no_refers_me'] === "true" ? 1 : 0);
		echo Mage::getSingleton('core/session')->getWRMNo();
		echo Mage::getSingleton('core/session')->getWRMId();
		echo Mage::getSingleton('core/session')->getWRMName();
	}
}
