 ...
 */
class Magestore_RewardPointsTransfer_Block_Adminhtml_RewardPointsTransfer_Renderer_Receiver
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        if (strpos($actionName, 'export') === 0) {
            return $row->getReceiverEmail();
        }
        return sprintf('<a target="_blank" href="%s">%s</a>',
            $this->getUrl('adminhtml/customer/edit', array('id' => $row->getReceiverCustomerId())),
            $row->getReceiverEmail()
        );
    }
}
