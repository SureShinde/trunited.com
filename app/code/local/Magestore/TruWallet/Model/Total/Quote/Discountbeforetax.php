 ...
 * @author      Magestore Developer
 */
class Magestore_TruWallet_Model_Total_Quote_Discountbeforetax extends Mage_Sales_Model_Quote_Address_Total_Abstract {

    protected $_hiddentBaseDiscount = 0;
    protected $_hiddentDiscount = 0;

    /**
     * Magestore_TruWallet_Model_Total_Quote_Discountbeforetax constructor.
     */
    public function __construct() {
        $this->setCode('truwallet_before_tax');
    }
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address) {
        ...
        if (Mage::getStoreConfig('truwallet/spend/tax', $quote->getStoreId()) == '1') {
            return $this;
        }
        ...
        $truwalletDiscount = Mage::getModel('truwallet/customer')
                ->getConvertedFromBaseCustomerCredit($baseTruwalletDiscount);
        ...
        //update session
        $session->setBaseCustomerCreditAmount($baseTruwalletDiscount);
        
        //update address
        $address->setBaseTruwalletHiddenTax($this->_hiddentBaseDiscount);
        $address->setTruWalletHiddenTax($this->_hiddentDiscount);

        $address->setGrandTotal($address->getGrandTotal() - $truwalletDiscount + $this->_hiddentBaseDiscount);
        $address->setBaseGrandTotal($address->getBaseGrandTotal() - $baseTruwalletDiscount + $this->_hiddentDiscount);
    ...
    public function _prepareDiscountCreditForAmount(Mage_Sales_Model_Quote_Address $address, $rate, $baseTruWalletForShipping) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    if(!$child->isDeleted()) {
                        ...
                        if(Mage::helper('tax')->priceIncludesTax()) {
                            ...
                            $child->setBaseTruwalletHiddenTax($child->getBaseTruwalletHiddenTax() + $baseHiddenTax);
                            $child->setTruWalletHiddenTax($child->getTruWalletHiddenTax() + $hiddenTax);
                        }
                    }
                }
            } else if ($item->getProduct()) {
                if(!$item->isDeleted()) {
                    ...
                    if(Mage::helper('tax')->priceIncludesTax()) {
                        ...
                        $this->_hiddentBaseDiscount += $baseHiddenTax;
                        $this->_hiddentDiscount += $hiddenTax;
                        $item->setBaseTruwalletHiddenTax($item->getBaseTruwalletHiddenTax() + $baseHiddenTax);
                        $item->setTruWalletHiddenTax($item->getTruWalletHiddenTax() + $hiddenTax);
                    }
                }
            }
        }
        if ($helper->getSpendConfig('shipping') && $baseTruWalletForShipping) {
            ...
            $baseShippingDiscount = min($baseTruWalletForShipping, $baseShippingPrice);
            ...
            $address->setBaseShippingTaxable($baseTaxableShippingAmount - $address->getBaseTruwalletDiscountForShipping());
            $address->setShippingTaxable($taxableShippingAmount - $address->getTruwalletDiscountForShipping());
            ...
            if(Mage::helper('tax')->shippingPriceIncludesTax()) {
                ...
                //update address
                $address->setBaseTruwalletShippingHiddenTax($this->round($this->calTax($baseTaxableShippingAmount, $taxShippingRate) - $this->calTax($address->getBaseShippingTaxable(), $taxShippingRate)));
                $address->setTruwalletShippingHiddenTax($this->round($this->calTax($taxableShippingAmount, $taxShippingRate) - $this->calTax($address->getShippingTaxable(), $taxShippingRate)));
            }
        }
        return $this;
    }
