...
class Magestore_TruWallet_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
	protected function _initAction(){
		$this->loadLayout()
			->_setActiveMenu('truwallet/customer')
	...
	public function editAction() {
		...
		$model  = Mage::getModel('truwallet/customer')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('truwallet/customer');

			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Item Manager'), Mage::helper('adminhtml')->__('Item Manager'));
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Item News'), Mage::helper('adminhtml')->__('Item News'));

			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
			$this->_addContent($this->getLayout()->createBlock('truwallet/adminhtml_customer_edit'))
				->_addLeft($this->getLayout()->createBlock('truwallet/adminhtml_customer_edit_tabs'));
		...
		if(!is_array($truwalletIds)){
			...
			try {
				foreach ($truwalletIds as $truwalletId) {
					$truwallet = Mage::getModel('truwallet/customer')->load($truwalletId);
					$truwallet->delete();
				}
	...
	public function massStatusAction() {
		...
		if(!is_array($truwalletIds)) {
			...
			try {
				foreach ($truwalletIds as $truwalletId) {
					$truwallet = Mage::getSingleton('truwallet/customer')
						->load($truwalletId)
	...
	public function exportCsvAction(){
		...
		$content	= $this->getLayout()->createBlock('truwallet/adminhtml_customer_grid')->getCsv();
