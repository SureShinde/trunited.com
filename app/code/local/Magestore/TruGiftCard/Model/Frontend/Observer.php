 ...
 */
class Magestore_TruGiftCard_Model_Frontend_Observer {
     ...
     * @param type $observer
     */
    public function trugiftcardPaymentMethod($observer) {
        ...
        if ($block instanceof Mage_Checkout_Block_Onepage_Payment_Methods || $block instanceof Magestore_Webpos_Block_Onepage_Payment_Methods) {
            ...
            $html_addcredit = $block->getLayout()->createBlock('trugiftcard/payment_form')->renderView();
        ...
        if (version_compare(Mage::getVersion(), '1.4.2', '>=')) {
            if ($paypalCart = $observer->getPaypalCart()) {
                ...
                if ($baseDiscount < 0.0001 && $salesEntity instanceof Mage_Sales_Model_Quote) {
                    $helper = Mage::helper('trugiftcard/spending');
                ...
                if ($baseDiscount > 0.0001) {
                    $paypalCart->updateTotal(
                            Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, (float) $baseDiscount, Mage::helper('trugiftcard')->__('Use points on spend')
                    );
                }
            }
            return $this;
        ...
        if ($salesEntity && $additional) {
            ...
            if ($baseDiscount > 0.0001) {
                ...
                $items[] = new Varien_Object(array(
                    'name' => Mage::helper('trugiftcard')->__('Trugiftcard on spend'),
                    'qty' => 1,
