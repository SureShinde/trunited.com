...
class Magestore_RewardPoints_Model_Observer
{
    ...
    public function salesOrderPlaceBefore($observer)
    {
        ...
        $balance = Mage::helper('rewardpoints/customer')->getProductCredit();
        if ($balance < $totalPointSpent) {
            throw new Mage_Core_Exception(Mage::helper('rewardpoints')->__(
                'Your credits balance is not enough to spend for this order'
            ));
        }
        ...
        if ($minPoint > $balance) {
            throw new Mage_Core_Exception(Mage::helper('rewardpoints')->__(
                'Minimum credits balance allows to redeem is %s',
                Mage::helper('rewardpoints/point')->formatProductCredit($minPoint, $quote->getStoreId())
            ));
        }
    ...
    public function customerRegisterSuccess($observer)
    {
        ...
        $transactions = Mage::helper('rewardpoints/transaction')->processTransaction($customer);
    ...
    {
        Mage::helper('rewardpoints/transaction')->checkExpiryDateTransaction();
    }
