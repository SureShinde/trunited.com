    ...
    public function fetch(Mage_Sales_Model_Quote_Address $address) {
        ...
        if ($amount = $address->getRewardpointsDiscount()) {
            if ($points = $address->getRewardpointsSpent() - $productPoints) {
                $title = Mage::helper('rewardpoints')->__('Truwallet (%s)', Mage::helper('rewardpoints/point')->formatProductCredit($points, $address->getQuote()->getStoreId())
                );
            } else {
                $title = Mage::helper('rewardpoints')->__('Truwallet on spend');
            }
