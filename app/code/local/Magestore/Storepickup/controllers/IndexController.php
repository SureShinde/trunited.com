...
class Magestore_Storepickup_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        Mage::helper('storepickup')->echoAllStoreCheckoutToJson();
        ...
        $from = urlencode('9360 Federal Blvd,Federal Heights, CO');
        $to = urlencode('9280 Federal Blvd, Federal Heights, CO 80260');
        $url ="https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=$from&destinations=$to";
        $ch = curl_init();
        ...
        zend_debug::dump($url);
        zend_debug::dump($result_array);
        zend_debug::dump($result_array->status);
        zend_debug::dump(trim($result_array->rows[0]->elements[0]->distance->text, " a..zA..Z"));
        zend_debug::dump($result_array->rows[0]->elements[0]->distance->text);
        zend_debug::dump(($result_array->rows[0]->elements[0]->distance->text * 1.609344).' km');
        zend_debug::dump(($result_array->rows[0]->elements[0]->distance->text * 1.609344 * 1000).' m');
    }
    ...
    public function updateDbAction()
    {
        ...
        $installer->run("
            DROP TABLE IF EXISTS {$setup->getTable('storepickup/distance')};
            CREATE TABLE {$setup->getTable('storepickup/distance')} (
            ...
            ALTER TABLE {$setup->getTable('storepickup/distance')} ADD UNIQUE `unique_index`(`destination_addresses`,`origin_addresses`);
		");
        $installer->endSetup();
        echo "success";
    }
