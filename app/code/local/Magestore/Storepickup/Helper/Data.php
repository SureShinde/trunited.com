...
{
    public function getDataConfig($field_name)
    {
        return Mage::getStoreConfig('carriers/storepickup/' . $field_name);
    }
    ...
    public function echoAllStoreCheckoutToJson($storeViewId = null)
    {
        ...
        if ($customer->getData('default_billing') > 0) {
            $address = Mage::getModel('customer/address')->load($customer->getData('default_billing'));
            $billing_address_data = Mage::getModel('checkout/session')->getQuote()->getBillingAddress()->getData();
            if ($billing_address_data != null && is_array($billing_address_data)) {
                $current_billing_address .= $billing_address_data['street'] . ', ' . $billing_address_data['city'] . ', ' . $billing_address_data['region'] . ' ' . $billing_address_data['postcode'] . ', ' . $billing_address_data['country_id'];
                Mage::log('BILLING ADDRESS ' . $current_billing_address, null, 'storepickup.log');
            } else if ($address->getId()) {
                $current_billing_address .= $address->getData('street') . ', ' . $address->getData('city') . ', ' . $address->getData('region') . ' ' . $address->getData('postcode') . ', ' . $address->getData('country_id');
            }
        }
        ...
        $filter_radius = $this->getDataConfig('filter_radius');
        $filter_measure = $this->getDataConfig('filter_measure');
        if ($filter_radius < 0 || $filter_radius == null)
            return false;
        ...
        $stores = array();
        foreach ($storeCollection as $store) {
            $dest = $store->getData('store_latitude') . ',' . $store->getData('store_longitude');
            $distance = $this->calculateDistance($billing_address, $dest, $filter_measure);
            if ($distance > 0 && $distance <= $filter_radius) {
                $store->setStoreId($storeViewId);
                $storeData = $store->getData();
                if (isset($images[$store->getId()])) {
                    $storeData['image_src'] = $images[$store->getId()];
        ...
        Mage::log('STORES ' . json_encode($stores), null, 'storepickup.log');
        if (sizeof($stores) > 0)
            return 'var listStoreJson = ' . Mage::helper('core')->jsonEncode($stores) . ';';
        else
            return false;
    ...
    public function calculateDistance($from, $to, $unit)
    {
        $_from = urlencode($from);
        $result = $this->checkResultDistance($from, $to, $unit);
        Mage::log('CHECK RESULT ' . $result, null, 'storepickup.log');
        if ($result != null)
            return $result;

        $google_key = Mage::getModel('storepickup/shipping_storepickup')->getConfigData('gkey');
        $url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=$_from&destinations=$to&key=$google_key";
        $rs = false;
        ...
        try {
            ...
            $this->saveDistance($from, $to, $unit, 0);
            return false;
        }
        curl_close($ch);
        Mage::log('DISTANCE URL ' . $url . ' - RESULT: ' . json_encode($result_array), null, 'storepickup.log');
        if ($result_array != null && strcasecmp($result_array->status, 'ok') == 0) {
            if (isset($result_array->rows[0]->elements[0]) && strcasecmp($result_array->rows[0]->elements[0]->status, 'ok') == 0) {
                $rs_meters = $result_array->rows[0]->elements[0]->distance->value;
                switch ($unit) {
                    case Magestore_Storepickup_Model_Status::DISTANCE_BY_MILES:
                        $rs = ($rs_meters * 1000) / 1.609344;
                        break;
                    case Magestore_Storepickup_Model_Status::DISTANCE_BY_FEET:
                        $rs = (($rs_meters * 1000) / 1.609344) * 5280;
                        break;
                    case Magestore_Storepickup_Model_Status::DISTANCE_BY_YARDS:
                        $rs = ((($rs_meters * 1000) / 1.609344) * 5280) / 3;
                        break;
                    case Magestore_Storepickup_Model_Status::DISTANCE_BY_KILOMETERS:
                        $rs = $rs_meters * 1000;
                        break;
                    case Magestore_Storepickup_Model_Status::DISTANCE_BY_METERS:
                        $rs = $rs_meters;
                        break;
                }
            }
        ...
        if(!$rs)
            ...
            $this->saveDistance($from, $to, $unit, $rs);

        return $rs;
    }

    public function saveDistance($from, $to, $unit, $distance)
    {
        if($distance > 0){
            $model = Mage::getModel('storepickup/distance');
            $data = array(
                'destination_addresses' => $from,
                'origin_addresses' => $to,
    ...
    public function checkResultDistance($from, $to, $unit)
    {
        $collection = Mage::getModel('storepickup/distance')->getCollection()
            ->addFieldToFilter('destination_addresses', $from)
            ->addFieldToFilter('origin_addresses', $to)
            ->getFirstItem()
            ;
        if($collection != null && $collection->getId())
        {
            $rs_meters = floatval($collection->getData('distance_value'));
            Mage::log('DISTANCE value ' . $collection->getData('distance_value') . ' - RESULT: ' . $rs_meters, null, 'storepickup.log');
            switch ($unit) {
                case Magestore_Storepickup_Model_Status::DISTANCE_BY_MILES:
                    return ($rs_meters * 1000) / 1.609344;
                case Magestore_Storepickup_Model_Status::DISTANCE_BY_FEET:
                    return (($rs_meters * 1000) / 1.609344) * 5280;
                case Magestore_Storepickup_Model_Status::DISTANCE_BY_YARDS:
                    return ((($rs_meters * 1000) / 1.609344) * 5280) / 3;
                case Magestore_Storepickup_Model_Status::DISTANCE_BY_KILOMETERS:
                    return $rs_meters * 1000;
                case Magestore_Storepickup_Model_Status::DISTANCE_BY_METERS:
                    return $rs_meters;
                default:
                    return null;
    ...
    function getDistance($addressFrom, $addressTo, $unit)
    {
        ...
        $gkey = Mage::getModel('storepickup/shipping_storepickup')->getConfigData('gkey');

        //Send request and receive json data
        $geocodeFrom = file_get_contents('https://maps.google.com/maps/api/geocode/json?address=' . $formattedAddrFrom . '&sensor=false&key=' . $gkey);
        $outputFrom = json_decode($geocodeFrom);
        $geocodeTo = file_get_contents('https://maps.google.com/maps/api/geocode/json?address=' . $formattedAddrTo . '&sensor=false&key=' . $gkey);
